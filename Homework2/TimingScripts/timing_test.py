...
# open file
with open("/Users/Kayleigh/Dropbox/IBRAIN/Homework2/sub-03/ses-test/func/sub-03_ses-test_task-linebisection_events.tsv", 'r') as f:
    s=f.read()
...
with open("timing_LCor.1D","w") as f:
	f.write(' '.join(timing_dic["LCor"]))
with open("timing_LIncor.1D","w") as f:
	f.write(' '.join(timing_dic["LIncor"]))
with open("timing_DResp.1D","w") as f:
	f.write(' '.join(timing_dic["DResp"]))
with open("timing_LNR.1D","w") as f:
	f.write(' '.join(timing_dic["LNR"]))
with open("timing_DNR.1D","w") as f:
